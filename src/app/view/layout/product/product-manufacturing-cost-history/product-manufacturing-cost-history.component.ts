...
export class ProductManufacturingCostHistoryComponent implements OnInit {
  ...
  displayedColumns: string[] = ['index', 'cost', 'creationTimestamp', 'action'];
  pageData: any[] = [];
  pageSizes = [5, 10, 15];
  ...
  assignPageData() {
    this.paginator.page.pipe(
      startWith({}),
      switchMap(() => {
        return this.manufacturingCostService.fetchPage(this.productId, this.searchedStartDate, this.searchedEndDate, this.paginator.pageIndex + 1, this.paginator.pageSize)
          .pipe(catchError(() => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        if (this.pageData.length == 1) {
          this.toastrService.warning("There must be at least one manufacturing cost, so it cannot be deleted.", "Manufacturing Cost Deletion");
          return;
        }
