...
export class CustomerListComponent implements OnInit {
  ...
  displayedColumnsWithAction: string[] = ['index', 'name', 'phone', 'action'];
  displayedColumns: string[] = ['index', 'name', 'phone'];
  pageData: any[] = [];
  pageSizes = [5, 10, 15];
  ...
  assignPageData() {
    this.paginator.page.pipe(
      startWith({}),
      switchMap(() => {
        return this.customerService.fetchPage(this.searchedKeyword, this.paginator.pageIndex + 1, this.paginator.pageSize)
          .pipe(catchError(() => {
  ...
  update(id: number): void {
    this.customerService.fetchById(id).subscribe({
        ...
        this.customerService.delete(id).subscribe({
          ...
          error: (error) => {
            if (error.status == HttpStatusCode.NotFound) {
              ...
              this.toastrService.error("There are orders which use this customer, so please delete them at first.", error.error.title);
            }
          }
        });
      }
