...
export class PickupListComponent implements OnInit {
  ...
  assignPageData() {
    this.paginator.page.pipe(
      ...
      this.pageData = data.map(obj => {
        return { 'index': ++index, ...obj, 'pickupDetails': true, 'action': true };
      });
  ...
  update(id: string): void {
    this.pickupService.fetchById(id).subscribe({
        ...
        this.pickupService.delete(id).subscribe({
          ...
          error: (error) => {
            if (error.status == HttpStatusCode.NotFound) {
              ...
              this.toastrService.error("There are pickup details contained in this pickup, so please delete them at first.", error.error.title);
            }
          }
        });
      }
  ...
  navigateToPickupDetails(id: string): void {
    this.pickupService.fetchById(id).subscribe({
      next: (response: Pickup) => {
        localStorage.setItem("pageIndex", this.paginator.pageIndex.toString());
        localStorage.setItem("pageSize", this.paginator.pageSize.toString());
