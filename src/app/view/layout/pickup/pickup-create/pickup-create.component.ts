...
@Component({
  ...
  styleUrls: ['./pickup-create.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class PickupCreateComponent implements OnInit {

  @ViewChild('paginator') paginator!: MatPaginator;
  @ViewChild(MatSort) sort!: MatSort;
  dataSource: MatTableDataSource<Order> = new MatTableDataSource<Order>();
  displayedColumns: string[] = ['index', 'id', 'orderDate', 'customer', 'createPickup'];
  pageData: any[] = [];
  pageSizes = [5, 10, 15];
  ...
  assignPageData() {
    this.paginator.page.pipe(
      startWith({}),
      switchMap(() => {
        return this.orderService.fetchNotFulfilledPage(this.searchedCustomerName, this.searchedStartDate, this.searchedEndDate, this.paginator.pageIndex + 1, this.paginator.pageSize)
          .pipe(catchError(() => {
      ...
      this.pageData = data.map(obj => {
        return { 'index': ++index, ...obj, 'createPickup': true };
      });
